...
pub async fn resolve(
    ...
    let drive_html = client
        ...
        .text()
        .await?;
    // Extract resolver URLs from the drive page in a scoped block so the
    // `Html` document (which contains !Send tendril data) is dropped before
    // the async fetch below — tokio::spawn requires the future to be Send.
    let (sportverse_url, gamerxyt_url) = {
        ...
        // Two resolver chains exist depending on the hubcloud mirror:
        //  - 4khdhub path: hubcloud.* page has a#download → https://sportverse.* → direct media
        //  - hdhub4u path: hubcloud.cx page has a link → https://gamerxyt.com/hubcloud.php → direct media
        let sportverse_url = document
            .select(&Selector::parse("a#download").unwrap())
            .filter_map(|node| node.value().attr("href"))
            .find(|href| href.starts_with("https://sportverse."))
            .map(str::to_string);

        let gamerxyt_url = document
            .select(&links)
            .filter_map(|node| node.value().attr("href"))
            .find(|href| href.starts_with("https://gamerxyt.com/hubcloud.php"))
            .map(str::to_string);

        (sportverse_url, gamerxyt_url)
    };

    let resolver_html = if let Some(sportverse_url) = sportverse_url {
        client
            .get(&sportverse_url)
            .send()
            .await?
            ...
            .text()
            .await?
    } else if let Some(gamerxyt_url) = gamerxyt_url {
        client
            .get(&gamerxyt_url)
            .send()
            .await?
        ...
        // No intermediate resolver: extract whatever direct links exist on the
        // drive page itself (pixeldrain vars, direct <a> links).
        drive_html.clone()
    };
