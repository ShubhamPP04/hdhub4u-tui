...
fn search_content(
    ...
    let content = if state.search_query.is_empty() {
        if state.is_tv_mode {
            ...
            "Search movies, series & more…".to_string()
        }
    } else {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState, theme: &Theme) {
        ...
        || (view == SearchViewState::Editing && state.search_results.is_empty())
    {
        ...
        let logo_text = if is_narrow {
            if state.is_tv_mode {
                r"  _   _   ____    _   _   _   _   ____    _  _     _   _
 | | | | |  _ \  | | | | | | | | | __ )  | || |   | | | |
 | |_| | | | | | | |_| | | | | | |  _ \  | || |_  | | | |
 |  _  | | |_| | |  _  | | |_| | | |_) | |__   _| | |_| |
 |_| |_| |____/  |_| |_|  \___/  |____/     |_|    \___/ TV"
                    .to_string()
            } else {
                r"  _   _   ____    _   _   _   _   ____    _  _     _   _
 | | | | |  _ \  | | | | | | | | | __ )  | || |   | | | |
...
╚═╝  ╚═╝╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚═════╝      ╚═╝ ╚═════╝"
            ...
            if state.is_tv_mode {
                r"  _  _   ___    _  _   _   _   ___   _ _    _   _ TV
 | || | |   \  | || | | | | | | _ ) | | |  | | | |
 | __ | | |) | | __ | | |_| | | _ \ |_  _| | |_| |
 |_||_| |___/  |_||_|  \___/  |___/   |_|   \___/"
                    .to_string()
            } else {
                r"  _  _   ___    _  _   _   _   ___   _ _    _   _
 | || | |   \  | || | | | | | | _ ) | | |  | | | |
 | __ | | |) | | __ | | |_| | | _ \ |_  _| | |_| |
 |_||_| |___/  |_||_|  \___/  |___/   |_|   \___/"
                    .to_string()
        ...
        let logo_width: u16 = if is_narrow {
            if state.is_tv_mode { 60 } else { 57 }
        } else if is_wide {
            if state.is_tv_mode { 61 } else { 58 }
        } else {
            if state.is_tv_mode { 52 } else { 50 }
        };
