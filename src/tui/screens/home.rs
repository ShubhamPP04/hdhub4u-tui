...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState, theme: &Theme) {
 ...
 ╚═╝   ╚═╝  ╚═════╝  ╚═╝   ╚═╝ ╚═╝  ╚═╝ ╚═╝  ╚══╝   ╚═╝╚═╝  ╚══╝  ╚═════╝  ╚═╝   ╚═╝"
            ...
            if state.is_tv_mode {
                r" _   _  _   _  ___  _   _  ___  ___  ___   ___  _  _ TV
| | | || | | |/ __|| | | || __|| _ \/ _ \| \| || || |
| |_| || |_| |\__ \| |_| || _| |   / (_) | .  || || |
 \___/  \___/ |___/ \___/ |___||_|_\\___/|_|\_||_||_|"
                    .to_string()
            } else {
...
| |_| || |_| |\__ \| |_| || _| |   / (_) | .  || || |
 \___/  \___/ |___/ \___/ |___||_|_\\___/|_|\_||_||_|"
        ...
        let logo_width: u16 = if is_narrow {
            if state.is_tv_mode { 46 } else { 44 }
        } else if is_wide {
            if state.is_tv_mode { 83 } else { 80 }
        } else {
            if state.is_tv_mode { 55 } else { 52 }
        };
