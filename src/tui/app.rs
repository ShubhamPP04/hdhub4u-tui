...
use crate::providers::{
    ...
    hdhub4u::{
        HdHub4uClient, details_to_moviebox_json as hdhub4u_details_to_moviebox_json,
...
impl App {
    pub fn new() -> Self {
        ...
        if let Some(config_dir) = dirs::config_dir() {
            ...
            if let Ok(config_str) = std::fs::read_to_string(config_path) {
                if let Ok(config_json) = serde_json::from_str::<serde_json::Value>(&config_str) {
                    ...
                    } else if config_json.get("active_provider").and_then(|v| v.as_str())
                        == Some(ProviderKind::HdHub4u.cache_key())
                    {
                        state.active_provider = ProviderKind::HdHub4u;
                    }
        ...
        Self {
            ...
            hdhub4u_client: HdHub4uClient::new(),
            action_sender,
            action_receiver,
        }
    }
    ...
    async fn handle_action(&mut self, action: Action) -> Option<()> {
        ...
        match action {
            ...
            Action::Search {
                ...
                let hdhub4u_client = self.hdhub4u_client.clone();
                let context = self.request_context();
                tokio::spawn(async move {
                    if !force_refresh {
                    ...
                    let result = match context.provider {
                        ...
                        ProviderKind::HdHub4u => hdhub4u_client
                            .search(&query_clone)
                            .await
                            .map(|items| hdhub4u_search_to_moviebox_json(&items))
                            .map_err(|error| error.to_string()),
                    };
                    match result {
                        Ok(res) => {
                ...
                tokio::spawn(async move {
                    ...
                    let result = match context.provider {
                        ...
                        ProviderKind::HdHub4u => hdhub4u_client
                            .details(&id_clone)
                            .await
                            .map(|details| hdhub4u_details_to_moviebox_json(&details))
                            .map_err(|error| error.to_string()),
                    };
                    match result {
                        Ok(details) => {
            ...
            Action::FetchPreview(id) => {
                ...
                if self.state.active_provider == ProviderKind::FourKHdHub
                    || self.state.active_provider == ProviderKind::HdHub4u
                {
                    self.state.preview_loading = false;
                    self.state.search_preview = None;
                    return None;
            ...
            Action::PlayStream(open_with) => {
                ...
                if self.state.active_provider == ProviderKind::HdHub4u {
                    if let Some(release) = self.get_selected_release() {
                        self.state.notify(
                            NotificationKind::Info,
                            "Preparing playback",
                        ...
                        tokio::spawn(async move {
                            match client.resolve_release(&release).await {
                                ...
                                Err(error) => {
                                    sender
                                        .send(Action::SetStatus(format!(
                                            "Error: HDHub4u source failed: {error}"
                                        )))
                                        .ok();
                                }
                            }
            ...
            Action::FetchEpisodeStreams {
                ...
                if context.provider == ProviderKind::HdHub4u {
                    let sender = self.action_sender.clone();
                    let client = self.hdhub4u_client.clone();
                    let id = subject_id.clone();
                    tokio::spawn(async move {
                            ...
                            Ok(releases) if !releases.is_empty() => {
                                sender
                                    .send(Action::EpisodeStreamsReady(
                                        ...
                                        season,
                                        episode,
                                        hdhub4u_releases_to_moviebox_json(&releases),
                                    ))
                                    .ok();
                            }
                            Ok(_) => {
